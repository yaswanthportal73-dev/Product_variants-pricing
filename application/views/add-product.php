...
tr.innerHTML=`
<td><select name="pricing[${i}][unit_id]" class="form-control unit-select" required>${opts}</select></td>
<td><input type="number" name="pricing[${i}][volume]" class="form-control volume-input" value="1" min="0.01" step="0.01"></td>
<td><input type="number" name="pricing[${i}][purchase_price]" class="form-control purchase-input" value="0.00" step="0.01"></td>
<td><input type="number" name="pricing[${i}][selling_price]" class="form-control selling-input" value="0.00" step="0.01"></td>
...
row.querySelectorAll('.volume-input, .purchase-input, .selling-input').forEach(el=>{
el.addEventListener('input', recalculateAllRows);
el.addEventListener('blur', recalculateAllRows);
});
}
...
const baseVol=parseFloat(baseRow.querySelector('.volume-input').value)||1;
const basePur=parseFloat(baseRow.querySelector('.purchase-input').value)||0;
const baseSel=parseFloat(baseRow.querySelector('.selling-input').value)||0;

rows.forEach((row,i)=>{
const volInput=row.querySelector('.volume-input');
const purInput=row.querySelector('.purchase-input');
const selInput=row.querySelector('.selling-input');
...
addRow();
const row=document.querySelectorAll('#pricingTable tbody tr')[<?php echo $idx; ?>];
row.querySelector('.unit-select').value='<?php echo $v->unit_id; ?>';
row.querySelector('.volume-input').value='<?php echo $v->volume; ?>';
row.querySelector('.purchase-input').value='<?php echo $v->purchase_price; ?>';
row.querySelector('.selling-input').value='<?php echo $v->selling_price; ?>';
<?php endforeach; ?>
recalculateAllRows();
<?php endif; ?>
});
